...
def plot_noise_curves(model: Model, 
    ...
    batch, target = batch_together(model.gen_batch_rand, reps=len(noise_std), n_batch=batch_size)
...
def plot_hidden_2D(model: BasicMLP, rows: Optional[int] = 2, cols: Optional[int] = None):
    try: W = model.W.detach().cpu().numpy()
    except: W = model.W_in.detach().cpu().numpy()
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            if idx < n_instances:
                ...
                axes[i, j].set_xlim(-1.3, 1.3)
                axes[i, j].set_ylim(-1.3, 1.3)
